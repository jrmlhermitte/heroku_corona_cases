...
from .data import canada_data, us_data
...
def cases_by_state(state_code):
    ...
    return render_template(
        "plotly_plot.html", plot_data=fig.to_javascript(), title=title_text
    )
...
def cases_canada_by_province(province):
    ...
    title_text = (
        f"Canada cases for province {province}. "
        "Last downloaded: "
        f" {last_updated}."
    )
...
def cases_for_united_states():
    ...
    last_updated = us_data.get_last_time_updated()
    title_text = (
        f"Cases for United States. Last downloaded: " f" {last_updated}."
    )
